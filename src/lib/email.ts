...
const TARGET_EMAIL = process.env.REVIEW_NOTIFICATION_EMAIL || "info@homeinteriorbandung.com";

/**
 * Sends an email notification to info@homeinteriorbandung.com when a new public review is submitted.
 *
...
export async function sendReviewEmailNotification(
  ...
  const from = process.env.SMTP_FROM || user || `Home Interior Bandung Website <noreply@homeinteriorbandung.com>`;
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #111; line-height: 1.5; border: 1px solid #eaeaea; border-radius: 8px; overflow: hidden;">
      <div style="background-color: #0b0b0b; color: #ffffff; padding: 20px 24px;">
        ...
        <p style="margin: 4px 0 0 0; font-size: 13px; color: #d4a373;">Home Interior Bandung Website</p>
      </div>
      ...
      <div style="background-color: #fafafa; padding: 14px 24px; font-size: 11px; color: #888; border-top: 1px solid #eaeaea;">
        Email ini dikirim otomatis oleh formulir ulasan publik website Home Interior Bandung ke ${TARGET_EMAIL}.
      </div>
    </div>
  `;
