...
const SEO_DESCRIPTION_SUFFIXES = [
  ...
  "Dirancang, diproduksi, dan dipasang langsung oleh tim Home Interior Bandung.",
...
function buildAlt(project: ManifestProject, index: number): string {
  ...
  const parts = [`${resolved.name} custom karya Home Interior Bandung`];
  const place = buildPlace(project);
  if (place) parts.push(`di ${place}`);
  ...
  beforeImage && afterImage
    ? {
        before: {
          ...
          alt: "Kondisi ruangan sebelum pengerjaan custom furniture Home Interior Bandung",
        ...
        after: {
          ...
          alt: "Ruangan yang sama setelah pemasangan custom furniture Home Interior Bandung",
